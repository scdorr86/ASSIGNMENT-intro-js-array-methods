...
const search = (event) => {
  ...
  const searchResult = referenceList.filter(item => 
    item.title.toLocaleLowerCase().includes(userInput) ||
    item.author.toLocaleLowerCase().includes(userInput) ||
    item.description.toLocaleLowerCase().includes(userInput)    
    )
